...
void send_write_request(int sock, const char *path, const char *data, int sync) {
    char buffer[BUFFER_SIZE];
    printf(YELLOW"Path: %s  Data: %s\n"RESET, path, data);
    snprintf(buffer, sizeof(buffer), "WRITE %s %s", path, data);
    if (sync) {
        // If sync flag is set, append "--SYNC"
        snprintf(buffer, sizeof(buffer), "WRITE %s %s --SYNC", path, data);
    } else {
        // If sync flag is not set, include all data up to the newline, no mode flag
        snprintf(buffer, sizeof(buffer), "WRITE %s %s --NOTSYNC", path, data);
    }
    printf(YELLOW"Sending: %s\n"RESET, buffer);
    send(sock, buffer, strlen(buffer), 0);

    if (sync) {
        // For synchronous writes, wait for server acknowledgment
        int bytes_received = recv(sock, buffer, BUFFER_SIZE - 1, 0);
        if (bytes_received < 0) {
        ...
        int bytes_received = recv(sock, buffer, BUFFER_SIZE - 1, MSG_DONTWAIT);
        if (bytes_received > 0) {
            buffer[bytes_received] = '\0';
            if (strncmp(buffer, "ACK", 3) == 0) {
                printf(GREEN "Asynchronous write request acknowledged by server\n" RESET);
            }
        } else if (bytes_received < 0 && errno != EAGAIN && errno != EWOULDBLOCK) {
            perror(RED "Error receiving acknowledgment for asynchronous write" RESET);
        }
    }
}
    ...
    if (strcmp(command, "COPY") == 0) {
        ...
        if (strcmp(command, "READ") == 0) {
            ...
            if(from_backup) {
                ...
                send_read_request(sock, store_path);  
            }
            ...
            if(from_backup) {
                ...
                send_write_request(sock, path, data, sync);
                ...
                send_write_request(sock, backup1, data, sync);
                ...
                send_write_request(sock, backup2, data, sync);
            }
            ...
            send_stream_request(sock, store_path);
        } else {
